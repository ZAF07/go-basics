...
type biter struct { // Struct is also private, so it cant be used standalone in any case which may introduce bugs
	biteForce            int
	entity.ILivingEntity // 💡 Embedded anonymous field containing the entity struct. This makes the Nonbiter struct looks like it inherits from the entity stuct. But it does not! It only allows us to call properties and methods of the embedded struct from the embedding struct as if it belongs to the embedding (outer) struct
